...
from pcgls import pcgls
from plsqr_b import plsqr_b
n = 32 #1024
A, b_bar, x = shaw(n)
np.random.seed(41997)
e = 1e-3 * np.random.randn(len(b_bar))
...
#
L, W = get_l(n, 1)
dum0, sm, dum1, dum2, dum3 = cgsvd(A, L.toarray())
# print('L.shape:', L.shape, 'W.shape:', W.shape, 'sm.shape:', sm.shape)
# [X, rho, eta, F] = pcgls(A, L.toarray(), W, b, 20, 0, sm)
# [X, rho, eta, F] = plsqr_b(A, L.toarray(), W, b, 20, 0, sm)
[X, rho, eta, F] = lsqr_b(A, b, 20, reorth=0, s=s)
# print('X:', X)
plt.figure()
plt.plot(x, 'r')
plt.plot(X[:,5], 'b')
...
from splsqr import splsqr

Vsp = dct(np.eye(32), norm='ortho').T
Vsp = Vsp[:, :6]
lambda_ = 1e-5
xsp = splsqr(A,b,lambda_,Vsp, 100, 1e-12, 0)
plt.figure()
plt.plot(x, 'r')
plt.plot(xsp[:,10], 'b')
plt.plot(xsp[:,99], 'g')
